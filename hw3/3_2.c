...
void counting_sort(int a[],int b[], int n, int *copy,int *comparison){
    ...
    int c[Range+1],i,j;
    // create a new array c[] to store the occurance of each element     
    for(i=0;i<=Range;i++)
        c[i]=0;
    //increment the occurance
    for(j=1;j<=n;j++)
        c[a[j]]= c[a[j]]+1;
    //Interchanging the index with the value
    for(i=1;i<=Range;i++)
        c[i]=c[i]+c[i-1];
    //populating the output array based on c index and value
    for(j=n;j>=1;j--){
        b[c[a[j]]]=a[j];
        c[a[j]]=c[a[j]]-1;
        *copy+=1;
